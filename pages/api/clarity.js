...
if (!q) return “”;
// Strip common question/filler words to isolate the ingredient name
const stripped = q
.replace(/^(is|are|can i|can i take|what about|tell me about|what is|how safe is|is it safe to (take|use|eat|drink)?|should i (take|use|avoid)?|i (take|use|eat|drink)|i’m taking|i am taking)\s+/i, “”)
.replace(/\s+(safe(\s+for\s+(breastfeeding|nursing|me|baby))?|okay|ok|dangerous|harmful|bad|good|allowed|fine|okay to (take|eat|use))??$/i, “”)
.replace(/\s+(while|during|when)\s+(breastfeeding|nursing|pregnant|postpartum).*$/i, “”)
.replace(/[?!.]+$/, “”)
.trim();
