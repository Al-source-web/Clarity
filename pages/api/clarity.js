...
function baseIngredientFromMessage(q) {
  ...
  var leadingPhrases = [
    "is it safe to take", "is it safe to eat", "is it safe to use",
    "can i take", "can i eat", "can i use", "can i drink",
    "should i take", "should i avoid", "should i use",
    "tell me about", "what about", "what is", "how safe is",
    "i am taking", "i'm taking", "i take", "i use", "i eat",
    "is it safe", "is", "are"
  ];
  for (var i = 0; i < leadingPhrases.length; i++) {
    var p = leadingPhrases[i] + " ";
    if (s.indexOf(p) === 0) { s = s.slice(p.length).trim(); break; }
  }
  // strip trailing context phrases
  var trailingPhrases = [
    "while breastfeeding", "while nursing", "during breastfeeding",
    "when breastfeeding", "when nursing", "for breastfeeding",
    "safe for breastfeeding", "safe for nursing", "safe for me",
    "safe for baby", "safe", "okay", "ok", "dangerous", "harmful",
    "bad", "good", "allowed", "fine"
  ];
  for (var j = 0; j < trailingPhrases.length; j++) {
    var t = " " + trailingPhrases[j];
    if (s.indexOf(t, s.length - t.length) !== -1) {
      s = s.slice(0, s.length - t.length).trim(); break;
    }
  }
  // take first segment before dash/colon, max 3 words
  var first = s.split(/[-:]/)[0].trim();
  var words = first.split(/\s+/).filter(Boolean);
...
export default async function handler(req, res) {
  ...
  if (req.query && req.query.action === "feedback") return handleFeedback(req, res);
  if (req.query && req.query.action === "loadmore") return handleLoadMore(req, res);
